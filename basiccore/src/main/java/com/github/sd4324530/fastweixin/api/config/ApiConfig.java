...
public final class ApiConfig extends Observable implements Serializable {
     ...
     */
    public ApiConfig(String appid, String secret,String wxname,RedisTemplate redisTemplate) {
        this(appid, secret, false,wxname,redisTemplate);
    }
     ...
     */
    public ApiConfig(String appid, String secret, boolean enableJsApi,String wxname,RedisTemplate redisTemplate) {
    ...
    public String getAccessToken() {
        ...
        try {
            ...
            if (template.getExpire(wxname+"_access_token")<100l && template.opsForValue().get(wxname+"_access_token_frashing").equals(false)) {
                template.opsForValue().set(wxname+"_access_token_frashing",true);
                LOG.debug("准备刷新token.............");
                initToken();
            }
        } catch (Exception e) {
            ...
            template.opsForValue().set(wxname+"_access_token_frashing",false);
        }
        return accessToken;
    }
    ...
    public String getJsApiTicket() {
        if (enableJsApi) {
            try {
                ...
                if (template.getExpire(wxname+"_js_access_token")<100l && template.opsForValue().get(wxname+"_js_access_token_frashing").equals(false)) {
                    template.opsForValue().set(wxname+"_js_access_token_frashing",true);
                    getAccessToken();
                    initJSToken();
                }
            } catch (Exception e) {
                ...
                //刷新工作出现有异常，将标识设置回false
                template.opsForValue().set(wxname+"_js_access_token_frashing",false);
            }
        } else {
            jsApiTicket = null;
        }
    ...
    private void initToken() {
        //如果缓存中有对应access_token 取access_token

        if(template.hasKey(this.wxname+"_access_token") && template.getExpire(this.wxname+"_access_token")>=100l){
            this.accessToken = (String) template.opsForValue().get(this.wxname+"_access_token");
        }else{
            ...
            NetWorkCenter.get(url, null, new NetWorkCenter.ResponseCallback() {
                ...
                public void onResponse(int resultCode, String resultJson) {
                    if (HttpStatus.SC_OK == resultCode) {
                        ...
                        template.opsForValue().set(wxname+"_access_token",accessToken,7200l, TimeUnit.SECONDS);
                        //设置通知点
                        setChanged();
    ...
    private void initJSToken() {
        ...
        if(template.hasKey(this.wxname+"_js_access_token") && template.getExpire(this.wxname+"_js_access_token")>100l){
            this.accessToken = (String) template.opsForValue().get(this.wxname+"_js_access_token");
        }else{
            ...
            NetWorkCenter.get(url, null, new NetWorkCenter.ResponseCallback() {
                ...
                public void onResponse(int resultCode, String resultJson) {
                    if (HttpStatus.SC_OK == resultCode) {
                        ...
                        template.opsForValue().set(wxname+"js_access_token",accessToken,7200l, TimeUnit.SECONDS);
                        //设置通知点
                        setChanged();
